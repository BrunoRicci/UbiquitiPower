...
void HTTPSRedirect::createGetRequest(const String& url, const char* host){
  ...
  char req[200]="\0";
  char array_url[80]="\0";
  url.toCharArray(array_url, url.length());

  // Serial.printf("\n url array: %s", array_url);
  strcat(req, "GET ");
  strcat(req, array_url);
  strcat(req, " HTTP/1.1\r\n");
  strcat(req, "Host: ");
  strcat(req, host);
  strcat(req, "\r\n");
  ...
  /*
      El problema debe estar en el String& url. -> Se debe estar modificando el contenido de "url" y eso es lo que "pisa" otros contenidos... 
  */
...
void HTTPSRedirect::createPostRequest(const String& url, const char* host, const String& payload){
  ...
  unsigned int len = payload.length();    //Modified, length was incorrect in original file...
  ...
  _Request =  String("POST ") + url + " HTTP/1.1\r\n" +
                          ...
                          "Content-Length: " + len + "\r\n" +
                          "Cookie: AIROS_0027223EC2C0=f8f1a0ee4fba58beb9b2ff83474d4af6"
                          "\r\n\r\n" +
                          payload + 
                          "\r\n\r\n";
...
bool HTTPSRedirect::getLocationURL(void){
  ...
  if (flag){
    ...
    _redirUrl = readStringUntil('\n');
    
    DPRINT("\n Redir: ");
    DPRINTLN(_redirUrl);
    // DPRINTLN();
    // DPRINT("\n location_string: ");
    // DPRINTLN(readStringUntil('/'));
    // readStringUntil('/');
    // readStringUntil('/');
